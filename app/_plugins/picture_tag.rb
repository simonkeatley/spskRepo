      ...
      if settings['markup'] == 'picturefill'
        ...
        source_keys.each { |source|
          if source == 'source_default'
            source_tags +=  "#{markdown_escape * 4}<img srcset=\"#{instance[source][:generated_src]}\" alt=\"#{html_attr['alt']}\">\n"
          else
            source_tags += "#{markdown_escape * 4}<source srcset=\"#{instance[source][:generated_src]}\" media=\"#{instance[source]['media']}\">\n"
          end
        }
